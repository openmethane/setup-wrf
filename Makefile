...
run: build  ## Run the required steps for the test domain
	docker run --rm -it -v $(PWD):/app -v /app/.venv setup-wrf python scripts/setup_for_wrf.py -c config/config.docker.json
	docker run --rm -it -v $(PWD):/app -v /app/.venv setup-wrf /app/data/runs/aust-test/main.sh
